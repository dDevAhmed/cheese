...
export enum EarnStatus { ACTIVE = 'active', PAUSED = 'paused', CLOSED = 'closed' }
...
export class EarnPosition {
  ...
  @Column({ name: 'principal_usdc', type: 'decimal', precision: 20, scale: 6, default: 0 })
  principalUsdc: string

  // Accumulated yield not yet credited
  @Column({ name: 'pending_yield_usdc', type: 'decimal', precision: 20, scale: 6, default: 0 })
  pendingYieldUsdc: string

  // Lifetime total yield earned
  @Column({ name: 'total_yield_usdc', type: 'decimal', precision: 20, scale: 6, default: 0 })
  totalYieldUsdc: string

  // APY applied at last calculation — stored for display
  @Column({ name: 'current_apy', type: 'decimal', precision: 6, scale: 4, default: 5.0 })
  currentApy: string
