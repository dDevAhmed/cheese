...
export class EarnService {
  ...
  constructor(
    @InjectRepository(EarnPosition) private readonly earnRepo:  Repository<EarnPosition>,
  ...
  async deposit(userId: string, dto: DepositEarnDto) {
    ...
    if (!user?.stellarPublicKey) throw new BadRequestException('Wallet not initialised')

    const amount = parseFloat(dto.amountUsdc)
    if (isNaN(amount) || amount < 1) throw new BadRequestException('Minimum earn deposit is 1 USDC')
    ...
    let position = await this.earnRepo.findOne({ where: { userId } })
    if (!position) {
      position = await this.earnRepo.save(this.earnRepo.create({
        userId,
    ...
    await this.earnRepo.update({ id: position.id }, {
  ...
  async withdraw(userId: string, dto: WithdrawEarnDto) {
    ...
    if (amount > available) throw new BadRequestException(`Maximum withdrawable: ${available.toFixed(6)} USDC`)
  ...
  async accrueYield() {
    ...
    const positions = await this.earnRepo.find({ where: { status: EarnStatus.ACTIVE } })
    let count = 0
    ...
    for (const position of positions) {
      try {
        ...
        this.logger.error(`Yield accrual failed for position ${position.id}: ${err.message}`)
      }
    }
