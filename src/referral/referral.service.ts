...
const REFERRAL_REWARD_USDC = 2.00   // $2 USDC per qualified referral
...
export class ReferralService {
  ...
  async getSummary(userId: string) {
    ...
    const qualified = referrals.filter((r) => r.status === ReferralStatus.QUALIFIED).length
    const rewarded  = referrals.filter((r) => r.status === ReferralStatus.REWARDED)
    const totalEarned = rewarded.reduce((sum, r) => sum + parseFloat(r.rewardUsdc || '0'), 0)
  ...
  async linkReferral(refereeId: string, referralCode: string): Promise<void> {
    ...
    const referrer = await this.userRepo.findOne({ where: { username: referrerUsername } })
  ...
  async qualifyReferral(refereeId: string): Promise<void> {
    ...
    await this.referralRepo.update({ id: referral.id }, { status: ReferralStatus.QUALIFIED })
  ...
  private async creditReward(referral: Referral): Promise<void> {
    try {
      ...
      await this.referralRepo.update({ id: referral.id }, {
      ...
      this.logger.log(`Referral reward credited: $${REFERRAL_REWARD_USDC} → ${referral.referrerId}`)
    } catch (err) {
