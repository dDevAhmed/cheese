...
export class BankTransferDto {
  @ApiProperty({ example: '0123456789', description: 'Destination Nigerian bank account number (10 digits)' })
  @IsString() @IsNotEmpty() @MinLength(10) @MaxLength(10)
  accountNumber: string

  @ApiProperty({ example: '058', description: 'Paystack bank code for the destination bank' })
  @IsString() @IsNotEmpty()
  bankCode: string

  @ApiProperty({ example: 'Tunde Okafor', description: 'Account holder name (must match resolved name to prevent misdirection)' })
  @IsString() @IsNotEmpty()
  accountName: string

  @ApiProperty({ example: '50000', description: 'Amount to send in NGN (as a numeric string)' })
  @IsNumberString()
  amountNgn: string
  ...
  @ApiProperty({ example: 'device-uuid-v4-here', description: 'Device ID of the signing device' })
  @IsString() @IsNotEmpty()
  deviceId: string
}
