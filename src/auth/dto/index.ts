...
export class SignupDto {
  @ApiProperty({ example: 'Tunde Okafor', description: 'Full legal name of the user' })
  @IsString() @IsNotEmpty() @MaxLength(80)
  fullName: string

  @ApiProperty({ example: 'tunde@example.com', description: 'Email address — used for OTP verification' })
  @IsEmail()
  email: string

  @ApiProperty({ example: '+2348012345678', description: 'Phone number in E.164 format' })
  @Matches(/^\+?[1-9]\d{9,14}$/, { message: 'Invalid phone number' })
  phone: string

  @ApiProperty({
    example: 'tunde_ok',
    description: 'Unique username (3–20 chars, letters/numbers/underscores only)',
  })
  @IsString() @MinLength(3) @MaxLength(20)
  @Matches(/^[a-zA-Z0-9_]+$/, { message: 'Username can only contain letters, numbers and underscores' })
  username: string

  @ApiProperty({ example: 'MyStr0ngPass!', description: 'Password (8–64 characters)' })
...
export class VerifyOtpDto {
  @ApiProperty({ example: 'tunde@example.com', description: 'Email address the OTP was sent to' })
  @IsEmail()
  email: string

  @ApiProperty({ example: '482910', description: '6-digit OTP code from email' })
...
export class ResendOtpDto {
  @ApiProperty({ example: 'tunde@example.com', description: 'Email address to resend OTP to' })
  @IsEmail()
  email: string
...
export class ForgotPasswordDto {
  @ApiProperty({ example: 'tunde@example.com', description: 'Email address to send reset OTP to' })
  @IsEmail()
  email: string
}

export class ResetPasswordDto {
  @ApiProperty({ example: 'tunde@example.com', description: 'Email address the reset OTP was sent to' })
  @IsEmail()
  email: string

  @ApiProperty({ example: '391847', description: '6-digit OTP from password reset email' })
  @IsString() @MinLength(6) @MaxLength(6)
  otp: string

  @ApiProperty({ example: 'NewStr0ngPass!', description: 'New password (8–64 characters)' })
  @IsString() @MinLength(8) @MaxLength(64)
  newPassword: string
}
...
export class VerifyPinDto {
  @ApiProperty({
    ...
    description: 'Client-computed HMAC-SHA256(pin, deviceId) in base64url — raw PIN never leaves the device',
  })
  @IsString() @IsNotEmpty()
  pinHash: string
...
export class ChangePinDto {
  @ApiProperty({
    ...
    description: 'HMAC-SHA256(currentPin, deviceId) — verifies ownership of existing PIN',
  })
  @IsString() @IsNotEmpty()
