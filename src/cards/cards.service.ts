...
import { VirtualCard, CardStatus, CardNetwork } from './entities/virtual-card.entity'
...
export class CardsService {
  ...
  constructor(
    @InjectRepository(VirtualCard) private readonly cardRepo: Repository<VirtualCard>,
  ...
  // ── GET /card ─────────────────────────────────────────────
  async getCard(userId: string): Promise<ReturnType<CardsService['formatCard']>> {
    let card = await this.cardRepo.findOne({ where: { userId, status: CardStatus.ACTIVE } })
      ?? await this.cardRepo.findOne({ where: { userId, status: CardStatus.FROZEN } })
  ...
  async freezeCard(userId: string) {
    ...
    if (card.status === CardStatus.FROZEN) throw new BadRequestException('Card is already frozen')
  ...
  async unfreezeCard(userId: string) {
    const card = await this.requireCard(userId)
    if (card.status === CardStatus.ACTIVE) throw new BadRequestException('Card is already active')
  ...
  async revealCvv(userId: string, dto: RevealCvvDto) {
    ...
    const card = await this.requireCard(userId)
    if (card.status === CardStatus.FROZEN) throw new ForbiddenException('Card is frozen')
