...
export class EmailService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.from     = config.get<string>('email.fromAddress', 'noreply@cheesewallet.app')
    this.fromName = config.get<string>('email.fromName',    'Cheese Wallet')
    this.replyTo  = config.get<string>('email.replyTo',     'support@cheesewallet.app')
  }
  ...
  private async send(payload: SendPayload): Promise<void> {
    if (!this.apiKey) {
      ...
      this.logger.warn(`[EMAIL — dev preview] To: ${payload.to} | Subject: ${payload.subject}`)
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        this.logger.error(`ZeptoMail send failed [${res.status}]: ${JSON.stringify(err)}`)
  ...
  async sendPasswordChanged(params: {
    ...
    const { subject, html } = passwordChanged({
      fullName:  params.fullName,
      changedAt: params.changedAt || new Date().toLocaleString('en-NG', { timeZone: 'Africa/Lagos' }),
